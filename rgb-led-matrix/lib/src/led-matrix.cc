...
Canvas *RGBMatrix::CreateCanvas(Canvas_ID id) {
  ...
  switch (id) {
    case Canvas_ID::RP2040:
      result = new FrameCanvas(new RP2040(_options.rows, _options.cols, &shared_pixel_mapper_));
      break;
  }

  result->framebuffer()->SetPWMBits(_options.pwm_bits);
  result->framebuffer()->SetBrightness(_options.brightness);
